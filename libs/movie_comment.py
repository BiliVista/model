...
def get_douban_comments(movie_id, start_idx=0, cookies=None):
    ...
    for comment in comments:
        ...
        publish_time = comment.find(class_='comment-info').find('span', class_='comment-time').text.strip() if comment.find(class_='comment-time') else "未知"
        loca = comment.find(class_='comment-info').find('span', class_='comment-location').text.strip() if comment.find(class_='comment-location') else "未知"
        rating = comment.find(class_='comment-info').find('span', class_='rating').get('title') if comment.find(class_='rating') else "无评分"
        content = comment.find(class_='short').text.strip()
        data.append({'用户名': username, '评分': rating, '评论内容': content, "地区": loca, "时间": publish_time})
...
def get_all_comments(movie_id, platform='maoyan', cookies=None, start_idx=0, batch_size=10, step=20,save_dir='tmp'):
    has_next = True
    all_data = []
    batch_count = 0
    ...
    while has_next:
        if platform == 'maoyan':
            ...
            comments, has_next = get_douban_comments(movie_id, start_idx, cookies)
        ...
        if len(all_data) >= batch_size or not has_next:
            ...
            save_comments_to_excel(combined_data, start_idx - len(all_data) * step, start_idx, platform,save_dir=save_dir,movie_id=movie_id)
            all_data.clear()
            batch_count += 1
